...
export async function handleCheckOrderStatus(ctx: ActionCtx): Promise<boolean> {
  ...
  const { tenantId, senderId, userName, profileUrl, settings, replyProviderName, replyModelName, salesTrigger } = ctx;
...
export async function handleBrowseCatalog(ctx: ActionCtx): Promise<boolean> {
  ...
  const { tenantId, senderId, settings } = ctx;
  ...
  if (alreadyShown) {
    ...
    await sendCatalogCategoryMenu(settings.pageAccessToken, senderId, tenantId, settings.pageId ?? undefined);
...
export async function handleSendImage(ctx: ActionCtx): Promise<boolean> {
  ...
  const { tenantId, senderId, userName, profileUrl, settings, allProducts, replyProviderName, replyModelName, salesTrigger } = ctx;
...
export async function handleStartOrder(ctx: ActionCtx): Promise<boolean> {
  ...
  const { tenantId, senderId, userName, profileUrl, settings, allProducts, salesTrigger, replyProviderName, replyModelName, lastUserMsgId } = ctx;
...
export async function handleConfirmOrder(ctx: ActionCtx): Promise<boolean> {
  ...
  const { tenantId, senderId, userName, profileUrl, settings, config, salesTrigger, replyProviderName, replyModelName, lastUserMsgId } = ctx;
  ...
  if (config.deliveryEnabled && resolvedWilaya) {
    ...
    await sendDeliveryOptions(settings.pageAccessToken, senderId, resolvedWilaya, tenantId, settings.pageId ?? undefined);
    await db.insert(conversationsTable).values({
      tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl,
...
export async function handleCreateOrder(ctx: ActionCtx): Promise<boolean> {
  ...
  const { tenantId, senderId, userName, profileUrl, settings, config, allProducts, salesTrigger, replyProviderName, replyModelName, lastUserMsgId } = ctx;
  ...
  if (config.deliveryEnabled && resolvedWilayaOA) {
    const deliveryIntroMsg = "شكراً! تم جمع بيانات طلبك. الآن اختر طريقة التوصيل:";
    await sendFbMessage(settings.pageAccessToken, senderId, deliveryIntroMsg, settings.pageId ?? undefined);
    await sendDeliveryOptions(settings.pageAccessToken, senderId, resolvedWilayaOA, tenantId, settings.pageId ?? undefined);
    await db.insert(conversationsTable).values({
      tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl,
      message: deliveryIntroMsg, sender: "bot",
      providerName: replyProviderName || null, modelName: replyModelName || null,
