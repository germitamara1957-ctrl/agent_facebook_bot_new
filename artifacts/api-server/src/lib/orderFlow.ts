...
export async function handleProductPayload(
  ...
  if (payloadAction === "ORDER_NOW") {
    const [targetProduct] = payloadProdId
      ? await db.select().from(productsTable).where(and(eq(productsTable.id, payloadProdId), eq(productsTable.tenantId, tenantId))).limit(1)
      : await db.select().from(productsTable).where(eq(productsTable.tenantId, tenantId)).limit(1);
    ...
    if (targetProduct) {
      if ((targetProduct.stockQuantity ?? 0) === 0) {
        ...
        await db.insert(conversationsTable).values({
          tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: null,
  ...
  if (payloadAction === "CONFIRM_ORDER") {
    ...
    const [appCfgOrd] = await db.select().from(aiConfigTable).where(eq(aiConfigTable.tenantId, tenantId)).limit(1);
    ...
    // تخفيض المخزون بعد تأكيد الطلب (الحد الأدنى 0 لمنع أرقام سالبة)
    if (session.productId && qtyOrd > 0) {
      await db.update(productsTable)
        .set({ stockQuantity: sql`GREATEST(0, ${productsTable.stockQuantity} - ${qtyOrd})` })
        .where(and(eq(productsTable.id, session.productId), eq(productsTable.tenantId, tenantId)));
    }
    ...
    const [existingLead] = await db.select().from(leadsTable)
      .where(and(eq(leadsTable.fbUserId, senderId), eq(leadsTable.tenantId, tenantId))).limit(1);
    if (existingLead) {
      await db.update(leadsTable).set({
        ...
        updatedAt: new Date(),
      }).where(and(eq(leadsTable.fbUserId, senderId), eq(leadsTable.tenantId, tenantId)));
    } else {
  ...
  if (payloadAction === "DELIVERY_HOME" || payloadAction === "DELIVERY_OFFICE") {
    const [deliveryAppConfig] = await db.select().from(aiConfigTable).where(eq(aiConfigTable.tenantId, tenantId)).limit(1);
    ...
    const [appConf] = await db.select().from(aiConfigTable).where(eq(aiConfigTable.tenantId, tenantId)).limit(1);
  ...
  if (payloadAction === "CHANGE_DELIVERY") {
    ...
    await sendDeliveryOptions(pageAccessToken, senderId, session.customerWilaya, tenantId, pageId);
  ...
  if (payloadAction === "PRICE_INFO") {
    const priceProducts = payloadProdId
      ? await db.select().from(productsTable).where(and(eq(productsTable.id, payloadProdId), eq(productsTable.tenantId, tenantId))).limit(1)
      : await db.select().from(productsTable).where(eq(productsTable.tenantId, tenantId)).limit(5);
  ...
  if (payloadAction === "BROWSE_UNCATEGORIZED") {
    await sendCatalogPage(pageAccessToken, senderId, { uncategorized: true }, 1, tenantId, pageId);
    return true;
  }
  ...
  if (payloadAction === "BROWSE_SUB") {
    ...
    return await handleBrowseSub(pageAccessToken, senderId, catId, tenantId, pageId);
  }

  // ── APPOINTMENT ──────────────────────────────────────────────────────────────
  if (payloadAction === "APPOINTMENT") {
    const [appConfig] = await db.select().from(aiConfigTable).where(eq(aiConfigTable.tenantId, tenantId)).limit(1);
  ...
  if (payloadAction === "DELIVERY") {
    const [appConfig] = await db.select().from(aiConfigTable).where(eq(aiConfigTable.tenantId, tenantId)).limit(1);
    const deliveryPrices = await db.select().from(deliveryPricesTable).where(eq(deliveryPricesTable.tenantId, tenantId));
  ...
  if (payloadAction === "FAQ") {
    const allFaqs = await db.select().from(faqsTable).where(eq(faqsTable.tenantId, tenantId)).limit(8);
    if (allFaqs.length === 0) {
  ...
  if (payloadAction === "FILTER_CATEGORY") {
    ...
    await sendCatalogPage(pageAccessToken, senderId, { category }, 1, tenantId, pageId);
    return true;
  }
  ...
  if (payloadAction === "FILTER_BRAND") {
    ...
    await sendCatalogPage(pageAccessToken, senderId, { brand }, 1, tenantId, pageId);
    return true;
  }
  ...
  if (payloadAction === "FILTER_PRICE_TIER") {
    ...
    await sendCatalogPage(pageAccessToken, senderId, { priceTier: tier }, 1, tenantId, pageId);
    return true;
  }
  ...
  if (payloadAction === "BROWSE_PAGE") {
    ...
    await sendCatalogPage(pageAccessToken, senderId, filters, page, tenantId, pageId);
    return true;
  }
  ...
  if (payloadAction === "DETAILS") {
    ...
    await db
      ...
      .values({ tenantId, fbUserId: senderId, productId: product.id, updatedAt: new Date() })
      .onConflictDoUpdate({
  ...
  if (payloadAction === "PREORDER_START") {
    ...
    await db.insert(preOrderSessionsTable)
      ...
      .onConflictDoUpdate({
        target: [preOrderSessionsTable.tenantId, preOrderSessionsTable.fbUserId],
        set: {
