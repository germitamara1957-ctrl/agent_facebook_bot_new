...
import { getTenantById } from "./tenantInit.js";
import {
  checkTenantActive,
...
export async function handleAiCall(p: AiCallParams): Promise<AiCallResult> {
  ...
  // ── فحص حالة المستأجر وحصة المحادثات ────────────────────────────────────────
  try {
    const tenant = await getTenantById(tenantId);
    if (tenant) {
      // 1. التحقق من الحالة (trial منتهي؟ موقوف؟)
      const statusCheck = checkTenantActive(tenant);
      if (!statusCheck.active) {
        const msg = getTenantStatusMessage(statusCheck.reason);
        try {
          await sendFbMessage(settings.pageAccessToken, senderId, msg, settings.pageId ?? undefined);
        } catch {}
        await db.insert(conversationsTable).values({
          tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl,
          message: msg, sender: "bot", timestamp: new Date(),
        });
        void logPlatformEvent("quota_blocked", senderId, `reason=${statusCheck.reason}`);
        return { outcome: "handled" };
      }

      // 2. التحقق من حصة المحادثات الشهرية
      const quotaCheck = await checkConversationQuota(tenantId);
      if (!quotaCheck.allowed) {
        const msg = quotaCheck.message ?? "تجاوزت حد المحادثات الشهرية. يرجى الترقية إلى خطة أعلى. 🔄";
        try {
          await sendFbMessage(settings.pageAccessToken, senderId, msg, settings.pageId ?? undefined);
        } catch {}
        await db.insert(conversationsTable).values({
        ...
        void logPlatformEvent("quota_blocked", senderId, `conversations=${quotaCheck.used}/${quotaCheck.limit}`);
        return { outcome: "handled" };
      }
    }
  } catch (quotaErr) {
    // الفشل في فحص الحصة لا يوقف الرد — نُسجّل ونكمل
    console.warn("[quota] Check failed (non-blocking):", (quotaErr as Error).message);
  }
  ...
  try {
    const aiResult    = await callAIWithMetadata(messages, systemPrompt, tenantId);
  ...
  // ── تسجيل استخدام محادثة واحدة ────────────────────────────────────────────────
  void incrementConversationUsage(tenantId).catch((err) => {
    console.warn("[quota] incrementConversationUsage failed:", (err as Error).message);
  });
