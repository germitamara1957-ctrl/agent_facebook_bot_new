...
export async function handlePreOrderSession(ctx: MsgCtx): Promise<boolean> {
  const { tenantId, senderId, userName, profileUrl, rawMessageText, pageAccessToken, pageId } = ctx;

  const [preOrderSession] = await db.select().from(preOrderSessionsTable)
    .where(and(
      eq(preOrderSessionsTable.tenantId, tenantId),
      eq(preOrderSessionsTable.fbUserId, senderId)
    )).limit(1);
  if (!preOrderSession) return false;

  await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: rawMessageText, sender: "user" });
  ...
  if (preOrderSession.step === "awaiting_name") {
    ...
    if (trimmedName.length < 2) {
      ...
      await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: nameErr, sender: "bot" });
      return true;
    }
    ...
    await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: askPhone, sender: "bot" });
    return true;
  }
  ...
  if (preOrderSession.step === "awaiting_phone") {
    ...
    if (!isValidPhoneNumber(rawPhone)) {
      ...
      await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: phoneErr, sender: "bot" });
      return true;
    }
    ...
    await db.delete(preOrderSessionsTable).where(and(
      eq(preOrderSessionsTable.tenantId, tenantId),
      eq(preOrderSessionsTable.fbUserId, senderId)
    ));
    const confirmMsg =
    ...
    await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: confirmMsg, sender: "bot" });
...
export async function handleDeliverySession(ctx: MsgCtx): Promise<boolean> {
  ...
  const [deliverySession] = await db.select().from(orderSessionsTable)
    .where(and(
      ...
      eq(orderSessionsTable.step, "choosing_delivery")
    ))
    .limit(1);
  ...
  if (isHomeIntent || isOfficeIntent) {
    await handleProductPayload(
      ...
      senderId, userName, pageAccessToken, pageId, tenantId
    );
  } else {
    if (deliverySession.customerWilaya) {
      await sendDeliveryOptions(pageAccessToken, senderId, deliverySession.customerWilaya, tenantId, pageId);
      await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: `🚚 اختر نوع التوصيل إلى ${deliverySession.customerWilaya}:`, sender: "bot" });
    } else {
      const chooseMsg = "يرجى اختيار نوع التوصيل:";
      await sendFbMessage(pageAccessToken, senderId, chooseMsg, pageId);
      await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: chooseMsg, sender: "bot" });
    }
  }
  return true;
}
...
export async function handleConfirmSession(ctx: MsgCtx): Promise<boolean> {
  const { tenantId, senderId, userName, profileUrl, rawMessageText, pageAccessToken, pageId, config } = ctx;
  ...
  if (isHomeMsg || isOfficeMsg) {
    ...
    await sendDeliveryOptions(pageAccessToken, senderId, confirmSession.customerWilaya, tenantId, pageId);
    await db.update(orderSessionsTable).set({ step: "choosing_delivery", updatedAt: new Date() })
      .where(and(
        eq(orderSessionsTable.tenantId, tenantId),
        eq(orderSessionsTable.fbUserId, senderId)
      ));
    await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: changeMsg, sender: "bot" });
  }
  return true;
}
...
export async function handleOrderMidFlow(ctx: MsgCtx): Promise<boolean> {
  ...
  if (isDeliveryPriceInquiry && config.deliveryEnabled) {
    ...
    if (wilayaToUse) {
      ...
      const [wp] = wilayaRec
        ? await db.select().from(deliveryPricesTable).where(and(
            eq(deliveryPricesTable.tenantId, tenantId),
            eq(deliveryPricesTable.wilayaId, wilayaRec.wilayaId)
          )).limit(1)
        : await db.select().from(deliveryPricesTable).where(and(
            eq(deliveryPricesTable.tenantId, tenantId),
            eq(deliveryPricesTable.wilayaName, wilayaToUse)
          )).limit(1);
      ...
      await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: deliveryInfoMsg, sender: "bot" });
    } else {
      ...
      await saveConversation({ tenantId, fbUserId: senderId, fbUserName: userName, fbProfileUrl: profileUrl, message: priceMsg, sender: "bot" });
    }
    return true;
  }
  ...
  if (isProductChangeIntent) {
    await db.delete(orderSessionsTable).where(and(
      eq(orderSessionsTable.tenantId, tenantId),
      eq(orderSessionsTable.fbUserId, senderId)
    ));
  }
