...
export async function callAIWithMetadata(
  ...
  const enabledProviders = await db
    ...
    .where(and(eq(aiProvidersTable.isEnabled, 1), eq(aiProvidersTable.tenantId, tenantId)))
    .orderBy(
  ...
  for (const provider of enabledProviders) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        await db.insert(providerUsageLogTable).values({
          tenantId, providerId: provider.id, success: 1, latencyMs: latency, error: null,
    ...
    await db.update(aiProvidersTable)
      ...
      .where(eq(aiProvidersTable.id, provider.id));
    await db.insert(providerUsageLogTable).values({
      tenantId, providerId: provider.id, success: 0, latencyMs: latency,
  ...
  if (enabledProviders.length === 0) {
    const [activeProvider] = await db
      ...
      .where(and(eq(aiProvidersTable.isActive, 1), eq(aiProvidersTable.tenantId, tenantId))).limit(1);
