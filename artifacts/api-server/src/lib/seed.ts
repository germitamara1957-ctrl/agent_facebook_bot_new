import { db, aiConfigTable, aiProvidersTable, productsTable, fbSettingsTable, adminUsersTable, faqsTable, availableSlotsTable, subscriptionPlansTable, subscriptionUsageTable, domainTemplatesTable, leadsTable, broadcastTemplatesTable, deliveryPricesTable, tenantsTable, superAdminsTable } from "@workspace/db";
...
const DEFAULT_DELIVERY_PRICES: Record<number, { officePrice: number; homePrice: number }> = {
  ...
  59: { officePrice: 600,  homePrice: 900  },
  60: { officePrice: 500,  homePrice: 800  },
  61: { officePrice: 400,  homePrice: 700  },
  62: { officePrice: 450,  homePrice: 750  },
  63: { officePrice: 450,  homePrice: 750  },
  64: { officePrice: 500,  homePrice: 800  },
  65: { officePrice: 650,  homePrice: 950  },
  66: { officePrice: 400,  homePrice: 700  },
  67: { officePrice: 450,  homePrice: 750  },
  68: { officePrice: 550,  homePrice: 850  },
  69: { officePrice: 600,  homePrice: 900  },
};
...
export async function runSeed(): Promise<void> {
  try {
    // ── 0. Global data: subscription plans (no tenant dependency) ─────────────
    ...
    // ── 1. Ensure default tenant (id=1) exists ────────────────────────────────
    // ON CONFLICT DO NOTHING (no column) handles ALL unique constraints:
    // id, slug, and owner_email.
    await db.execute(
      `INSERT INTO tenants (id, name, slug, owner_email, plan, status)
       VALUES (1, 'Default', 'default', 'admin@default.local', 'agency', 'active')
       ON CONFLICT DO NOTHING`
    );
    // Reset sequence so new tenant registrations start after the highest id.
    await db.execute(
      `SELECT setval('tenants_id_seq', GREATEST(1, (SELECT MAX(id) FROM tenants)), true)`
    );

    // Find the actual default tenant id (might differ if email/slug conflict existed).
    const defaultTenantRow = await db.execute(
      `SELECT id FROM tenants WHERE slug = 'default' OR owner_email = 'admin@default.local' ORDER BY id LIMIT 1`
    );
    const defaultTenantId: number = (defaultTenantRow.rows[0] as { id: number } | undefined)?.id ?? 1;
    console.log(`[seed] Default tenant ensured (id=${defaultTenantId})`);
    ...
    if ((adminCount?.value ?? 0) === 0) {
      ...
      await db.insert(adminUsersTable).values({ tenantId: defaultTenantId, username: adminUser, passwordHash: hash });
    }

    // ── Super Admin (platform-level) ─────────────────────────────────────────
    const [superAdminCount] = await db.select({ value: count() }).from(superAdminsTable);
    if ((superAdminCount?.value ?? 0) === 0) {
      const superUser = process.env["SUPER_ADMIN_USERNAME"] || "superadmin";
      let   superPass = process.env["SUPER_ADMIN_PASSWORD"] ?? "";

      if (!superPass) {
        superPass = "superadmin123";
        console.warn(
          "[seed] ⚠️  SUPER_ADMIN_PASSWORD not set — using default 'superadmin123'.\n" +
          "           Set SUPER_ADMIN_PASSWORD env var in production!"
        );
      }
      ...
      console.log(`[seed] Super admin '${superUser}' created`);
    }

    const [configCount] = await db.select({ value: count() }).from(aiConfigTable);
    if ((configCount?.value ?? 0) === 0) {
      await db.insert(aiConfigTable).values({ tenantId: defaultTenantId,
    ...
    if ((providerCount?.value ?? 0) === 0) {
      await db.insert(aiProvidersTable).values(PROVIDERS.map((p) => ({ ...p, tenantId: defaultTenantId })));
    ...
    if ((productCount?.value ?? 0) === 0) {
      ...
      await db.insert(productsTable).values({ tenantId: defaultTenantId,
    ...
    if ((faqCount?.value ?? 0) === 0) {
      await db.insert(faqsTable).values(DEFAULT_FAQS.map((f) => ({ ...f, tenantId: defaultTenantId })));
    ...
    if ((slotCount?.value ?? 0) === 0) {
      await db.insert(availableSlotsTable).values(DEFAULT_SLOTS.map((s) => ({ ...s, tenantId: defaultTenantId })));
    ...
    if ((leadCount?.value ?? 0) === 0) {
      await db.insert(leadsTable).values(SAMPLE_LEADS.map((l) => ({ ...l, tenantId: defaultTenantId })));
    ...
    if ((btCount?.value ?? 0) === 0) {
      await db.insert(broadcastTemplatesTable).values(DEFAULT_BROADCAST_TEMPLATES.map((t) => ({ ...t, tenantId: defaultTenantId })));
    ...
    for (const [wilayaIdStr, prices] of Object.entries(DEFAULT_DELIVERY_PRICES)) {
      ...
      const result = await db
        ...
        .where(
          and(
            eq(deliveryPricesTable.tenantId, defaultTenantId),
