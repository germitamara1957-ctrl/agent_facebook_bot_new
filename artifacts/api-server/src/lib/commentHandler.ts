...
async function saveProductContext(senderId: string, productId: number, productName: string, tenantId: number = 1): Promise<void> {
  try {
    await db
      .insert(userProductContextTable)
      .values({ tenantId, fbUserId: senderId, productId, updatedAt: new Date() })
      .onConflictDoUpdate({
  ...
  try {
    ...
    if (postText) {
      const allProducts = await db
        .select()
        .from(productsTable)
        .where(and(eq(productsTable.status, "available"), eq(productsTable.tenantId, tenantId)));
      ...
      if (match) {
        ...
        const [full] = await db
          .select()
          .from(productsTable)
          .where(and(eq(productsTable.id, match.id), eq(productsTable.tenantId, tenantId)))
          .limit(1);
    ...
    if (config.sendDmOnComment && senderId && settings.pageAccessToken) {
      ...
      try {
        ...
        if (matchedProduct && senderId) {
          await saveProductContext(senderId, matchedProduct.id, matchedProduct.name, tenantId);
        }
    ...
    await db.insert(commentsLogTable).values({
      tenantId, postId, commentId, fbUserId: senderId, fbUserName: userName,
