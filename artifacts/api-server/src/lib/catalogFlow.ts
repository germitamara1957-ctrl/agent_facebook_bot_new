...
export async function sendDeliveryOptions(
  ...
  const [wp] = wilayaRecord
    ? await db.select().from(deliveryPricesTable)
        .where(and(eq(deliveryPricesTable.tenantId, tenantId), eq(deliveryPricesTable.wilayaId, wilayaRecord.wilayaId))).limit(1)
    : await db.select().from(deliveryPricesTable)
        .where(and(eq(deliveryPricesTable.tenantId, tenantId), eq(deliveryPricesTable.wilayaName, wilayaName))).limit(1);
  const [appConf] = await db.select().from(aiConfigTable)
    .where(eq(aiConfigTable.tenantId, tenantId)).limit(1);
...
export async function sendCatalogCategoryMenu(
  ...
  const rootCats = await db.select()
    ...
    .where(and(eq(productCategoriesTable.tenantId, tenantId), isNull(productCategoriesTable.parentId)))
  ...
  const productCatRows = await db
    ...
    .from(productsTable)
    .where(and(eq(productsTable.tenantId, tenantId), eq(productsTable.status, "available")));
  ...
  if (catsWithProducts.length === 0) {
    ...
    await sendCatalogPage(pageAccessToken, senderId, {}, 1, tenantId, pageId);
    return;
  }
  ...
  const uncatProducts = await db.select({ id: productsTable.id })
    .from(productsTable)
    .where(and(eq(productsTable.tenantId, tenantId), eq(productsTable.status, "available"), isNull(productsTable.category)))
    .limit(1);
...
export async function sendCatalogPage(
  ...
  const conditions = [eq(productsTable.tenantId, tenantId), eq(productsTable.status, "available")];
          ...
          if (img.startsWith("data:")) {
            ...
            if (cdnUrl) {
              ...
              await db.update(productsTable)
                ...
                .where(and(eq(productsTable.id, p.id), eq(productsTable.tenantId, tenantId)))
                .catch(() => {/* non-critical */});
...
export async function handleBrowseSub(
  ...
  const [thisCat] = await db.select()
    .from(productCategoriesTable)
    .where(and(eq(productCategoriesTable.tenantId, tenantId), eq(productCategoriesTable.id, catId)))
    .limit(1);
  ...
  const children = await db.select()
    .from(productCategoriesTable)
    .where(and(eq(productCategoriesTable.tenantId, tenantId), eq(productCategoriesTable.parentId, catId)))
    .orderBy(productCategoriesTable.id);
  ...
  if (children.length > 0) {
    ...
    if (childrenWithProducts.length === 0) {
      ...
      await sendCatalogPage(pageAccessToken, senderId, { category: fullPath, categoryLeafName: thisCat.name }, 1, tenantId, pageId);
    } else {
