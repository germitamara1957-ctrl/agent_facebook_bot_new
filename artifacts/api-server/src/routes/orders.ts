...
router.get("/orders/count", async (req, res): Promise<void> => {
  const tenantId = (req as AuthenticatedRequest).user?.tenantId ?? 1;
  const [totalResult] = await db.select({ value: count() }).from(ordersTable).where(eq(ordersTable.tenantId, tenantId));
  const [pendingResult] = await db
    .select({ value: count() })
    .from(ordersTable)
    .where(and(eq(ordersTable.tenantId, tenantId), eq(ordersTable.status, "pending")));
  ...
  if (newLabel && updated.fbUserId) {
    try {
      await db.update(leadsTable)
        ...
        .where(and(eq(leadsTable.fbUserId, updated.fbUserId), eq(leadsTable.tenantId, tenantId)));
    } catch (e) {
...
router.get("/orders/export", async (req, res): Promise<void> => {
  ...
  const wilayaPrices = await db.select().from(deliveryPricesTable).where(eq(deliveryPricesTable.tenantId, tenantId));
...
router.delete("/orders/:id", async (req, res): Promise<void> => {
  ...
  const raw = Array.isArray(req.params["id"]) ? req.params["id"][0] : req.params["id"];
  const id = parseInt(raw!, 10);
  await db.delete(ordersTable).where(and(eq(ordersTable.id, id), eq(ordersTable.tenantId, tenantId)));
...
router.post("/orders/bulk-delete", async (req, res): Promise<void> => {
  ...
  await db.delete(ordersTable).where(and(inArray(ordersTable.id, ids), eq(ordersTable.tenantId, tenantId)));
