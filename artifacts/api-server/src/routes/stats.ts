...
router.get("/stats", async (req, res): Promise<void> => {
  ...
  const [sessionsTodayRow] = await db
    ...
    .where(and(
      ...
      eq(conversationsTable.sender, "user"),
      gte(conversationsTable.timestamp, todayStart),
      lt(conversationsTable.timestamp, tomorrowStart)
    ));
  ...
  const [sessionsWeekRow] = await db
    .select({ value: countDistinct(conversationsTable.fbUserId) })
    .from(conversationsTable)
    .where(and(eq(conversationsTable.tenantId, tenantId), eq(conversationsTable.sender, "user"), gte(conversationsTable.timestamp, weekStart)));
  const sessionsWeek = { value: sessionsWeekRow?.value ?? 0 };

  const [sessionsMonthRow] = await db
    .select({ value: countDistinct(conversationsTable.fbUserId) })
    .from(conversationsTable)
    .where(and(eq(conversationsTable.tenantId, tenantId), eq(conversationsTable.sender, "user"), gte(conversationsTable.timestamp, monthStart)));
  const sessionsMonth = { value: sessionsMonthRow?.value ?? 0 };

  const [ordersTotal] = await db.select({ value: count() }).from(ordersTable).where(eq(ordersTable.tenantId, tenantId));
  const [ordersPending] = await db.select({ value: count() }).from(ordersTable).where(and(eq(ordersTable.tenantId, tenantId), eq(ordersTable.status, "pending")));

  const [appointmentsTotal] = await db.select({ value: count() }).from(appointmentsTable).where(eq(appointmentsTable.tenantId, tenantId));
  const [appointmentsPending] = await db.select({ value: count() }).from(appointmentsTable).where(and(eq(appointmentsTable.tenantId, tenantId), eq(appointmentsTable.status, "pending")));

  const [commentsToday] = await db
    .select({ value: count() })
    .from(commentsLogTable)
    .where(and(eq(commentsLogTable.tenantId, tenantId), sql`${commentsLogTable.timestamp} >= ${todayStart}`));
  ...
  }).from(aiConfigTable).where(eq(aiConfigTable.tenantId, tenantId)).limit(1);

  const peakHoursRaw = await db.execute(sql`
    SELECT EXTRACT(HOUR FROM timestamp)::int as hour, COUNT(*)::int as count
    FROM conversations
    WHERE sender = 'user' AND tenant_id = ${tenantId}
    GROUP BY hour
    ORDER BY hour
  `);
  ...
  const topProductsRaw = await db
    ...
    .from(ordersTable)
    .where(and(eq(ordersTable.tenantId, tenantId), isNotNull(ordersTable.productName)))
  ...
  const sentimentRaw = await db
    ...
    .from(conversationsTable)
    .where(and(eq(conversationsTable.tenantId, tenantId), isNotNull(conversationsTable.sentiment)))
  ...
  const [ordersToday] = await db
    ...
    .where(and(
      eq(ordersTable.tenantId, tenantId),
      sql`${ordersTable.createdAt} >= ${todayStart} AND ${ordersTable.createdAt} < ${tomorrowStart}`
    ));
  ...
  const [errorsPreventedRow] = await db
    ...
    .where(and(eq(platformEventsTable.tenantId, tenantId), eq(platformEventsTable.eventType, "lost_risk_prevented"), gte(platformEventsTable.createdAt, todayStart)));
  ...
  const [humanInterventionsRow] = await db
    ...
    .from(conversationsTable)
    .where(and(eq(conversationsTable.tenantId, tenantId), eq(conversationsTable.isPaused, 1), gte(conversationsTable.timestamp, todayStart)));
  ...
  const [botOrdersRow] = await db
    ...
    .where(and(
      eq(conversationsTable.tenantId, tenantId),
      eq(conversationsTable.convertedToOrder, 1),
      eq(conversationsTable.conversionSource, "bot"),
      gte(conversationsTable.timestamp, todayStart)
    ));
