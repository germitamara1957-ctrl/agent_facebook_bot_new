...
import { checkProductQuota } from "../lib/quotaGuard.js";
...
router.get("/products", async (req, res): Promise<void> => {
  ...
  const rows = await db.select().from(productsTable).where(eq(productsTable.tenantId, tenantId)).orderBy(productsTable.id);
  res.json(rows);
});
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const quota = await checkProductQuota(tenantId);
    if (!quota.allowed) {
      res.status(403).json({ message: quota.message ?? "تجاوزت الحد الأقصى للمنتجات في خطتك. يرجى الترقية." });
      return;
    }
