...
router.get("/fb-settings", async (req, res): Promise<void> => {
  const tenantId: number = (req as { user?: { tenantId?: number } }).user?.tenantId ?? 1;
  let [settings] = await db.select().from(fbSettingsTable).limit(1);
  if (!settings) {
    [settings] = await db.insert(fbSettingsTable).values({ tenantId }).returning();
  }
  res.json({
    ...settings,
...
router.post("/fb-settings", async (req, res): Promise<void> => {
  ...
  if (!existing) {
    const [created] = await db.insert(fbSettingsTable).values({ tenantId, ...values }).returning();
    await rDel("settings");
    saved = created;
  } else {
