...
router.get("/ai-config", async (req, res): Promise<void> => {
  ...
  let [config] = await db.select().from(aiConfigTable).where(eq(aiConfigTable.tenantId, tenantId)).limit(1);
  if (!config) {
    [config] = await db
      .insert(aiConfigTable)
      .values({ tenantId, botName: "مساعد المتجر", language: "auto", currency: "DZD" })
      .returning();
  }
  res.json(config);
});
...
router.get("/ai-config/abandoned-cart-stats", async (req, res): Promise<void> => {
  const tenantId: number = (req as AuthenticatedRequest).user?.tenantId ?? 1;
  const [totalRow] = await db.select({ value: count() }).from(productInquiriesTable).where(eq(productInquiriesTable.tenantId, tenantId));
  const [sentRow] = await db.select({ value: count() }).from(productInquiriesTable).where(and(eq(productInquiriesTable.tenantId, tenantId), eq(productInquiriesTable.reminderSent, 1)));
  const [convertedRow] = await db.select({ value: count() }).from(productInquiriesTable).where(and(eq(productInquiriesTable.tenantId, tenantId), eq(productInquiriesTable.converted, 1)));
