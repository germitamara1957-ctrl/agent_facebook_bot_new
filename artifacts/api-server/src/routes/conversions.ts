...
router.get("/conversions", async (req, res): Promise<void> => {
  ...
  const [totalOrdersRow] = await db.select({ value: count() }).from(ordersTable).where(eq(ordersTable.tenantId, tenantId));
  ...
  const [sessionsTodayRow] = await db
    ...
    .where(and(
      eq(conversationSessionsTable.tenantId, tenantId),
      sql`${conversationSessionsTable.createdAt} >= ${todayStart}`
    ));
  ...
  const triggerBreakdownRaw = await db
    ...
    .from(conversationsTable)
    .where(and(eq(conversationsTable.tenantId, tenantId), eq(conversationsTable.convertedToOrder, 1)))
