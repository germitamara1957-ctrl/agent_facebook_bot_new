...
const LABEL_OPTIONS = [
  ...
  { value: "cold", label: "بارد / Cold", color: "bg-muted text-muted-foreground" },
...
export default function Conversations() {
  ...
  const { data: messages = [], isLoading: loadingMsgs } = useGetConversation(selectedUserId || "", {
    query: { enabled: !!selectedUserId, queryKey: [`/api/conversations/${selectedUserId}`] },
  });
  ...
  const handleSaveNote = async () => {
    ...
    try {
      ...
      await fetch(`/api/conversations/${selectedUserId}/note`, {
        method: "PATCH",
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto flex flex-col gap-4" style={{ height: "calc(100vh - 80px)" }}>
      <div className="flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" size="sm" onClick={handleRefresh} disabled={fetchingList} className="rounded-xl bg-card shadow-sm border-border/50 hover:bg-muted/40">
            <RefreshCw className={`w-4 h-4 me-2 ${fetchingList ? 'animate-spin' : ''}`} /> تحديث
          </Button>
        </div>
      </div>

      <Card className="flex-1 flex overflow-hidden border-border/50 shadow-md shadow-black/10 rounded-2xl min-h-0">
        <div className="w-1/3 border-e border-border/50 bg-muted/30 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-border/50 bg-card/50 backdrop-blur-sm z-10 sticky top-0">
            <h3 className="font-bold text-sm text-muted-foreground">قائمة العملاء</h3>
          </div>
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {loadingList ? (
              [1,2,3,4].map(i => <div key={i} className="h-20 bg-muted rounded-xl animate-pulse m-1" />)
            ) : users.length === 0 ? (
                  ...
                  <button
                    ...
                    className={`w-full text-start p-4 rounded-xl transition-all duration-200 ${
                      selectedUserId === u.fbUserId
                        ? "bg-card shadow-md shadow-black/10 border border-primary/20 ring-1 ring-primary/20"
                        : "hover:bg-muted border border-transparent"
                    }`}
                  >
                    <div className="flex justify-between items-start mb-1">
                      <span className={`font-bold text-sm truncate pe-2 ${selectedUserId === u.fbUserId ? 'text-primary' : 'text-foreground/80'}`}>
                        {u.fbUserName || "مستخدم"}
                      </span>
        ...
        <div className="w-2/3 flex flex-col bg-[#F8FAFC]">
          {selectedUserId && selectedUser ? (
            <>
              <div className="p-4 border-b border-border/50 bg-card shadow-sm flex items-center justify-between z-10 gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="font-bold text-foreground">{selectedUser.fbUserName}</h2>
                ...
                {loadingMsgs ? (
                  ...
                  messages.map((msg) => {
                    ...
                    if (isUser) {
                      ...
                      avatarClasses = "bg-border text-muted-foreground";
                      ...
                      bubbleClasses = "bg-card text-foreground rounded-2xl rounded-tr-sm border border-border/50";
              ...
              {isSelectedPaused ? (
                <div className="p-4 border-t border-border/50 bg-card shadow-inner">
                ...
                <div className="p-4 border-t border-border/50 bg-card/80 flex items-center justify-center gap-3">
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground/70 bg-muted/30">
              <MessageSquare className="w-16 h-16 mb-4 text-muted-foreground/30" />
              <p className="text-lg font-medium text-muted-foreground">اختر محادثة لعرضها</p>
            </div>
          )}
        </div>
      </Card>
