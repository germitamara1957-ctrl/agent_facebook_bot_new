...
async function deletePreOrder(id: number) {
  const res = await fetch(`/api/pre-orders/${id}`, { method: "DELETE" });
    ...
    <div className="p-6 space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">الطلبات المسبقة</h1>
            <p className="text-sm text-muted-foreground">إدارة طلبات المنتجات غير المتاحة</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { key: "all", label: "الكل", color: "from-gray-50 to-gray-100 border-border" },
          ...
          >
            <div className="text-2xl font-bold text-foreground">{counts[s.key as keyof typeof counts]}</div>
      ...
      {/* Table */}
      {isLoading ? (
        <div className="text-center py-16 text-muted-foreground/70">جارٍ التحميل...</div>
      ) : filtered.length === 0 ? (
